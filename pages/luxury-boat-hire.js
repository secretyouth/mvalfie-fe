...
const LuxuryBoatHire = ({ page }) => {
    ...
    return (
        <div className={st.container} id="top">
            <Head>
                <title key="title">Luxury Boat Hire | {process.env.Title || 'Alfie & Co'}</title>
                <meta property="og:url" content="https://www.mvalfieandco.com.au/luxury-boat-hire" key="og-url" />
            </Head>
                                ...
                                return (
                                    <>
                                        ...
                                        <div className="flex justify-content-between align-items-center mt-4 about-btns">
                                            <button className="btn secondary">Booking Enquiry</button>
                                            <button className="btn secondary">Become An Owner</button>
                                        </div>
                                    </>
                                )
                            default:
...
export async function getServerSideProps() {
    const pageContent = await api.get('luxury-boat-hire')

    // Pass post data to the page via props
    return { props: { page: pageContent.data } }
}

export default LuxuryBoatHire
