    ...
    return (
        <section className="boats about-boats gold-coast-main" id="boats">
            <div className="gold-coast-content-block">
                ...
                {data.description && <div className="mb-4 markdown content-p text-light" dangerouslySetInnerHTML={{ __html: marked(data.description) }} />}
            </div>
                ...
                {data.boat &&
                    <div className="boat-block text-md" style={{ backgroundImage: `url(${data.boat.Main_image.url})` }}>
                        <div className="internal">
                            ...
                            {component}
                            <a href={`/boats/${data.boat.Page_slug}`} className="btn block primary">Explore {data.boat.Boat_name}</a>
                            {
                                quick_view &&
